...
def extractPidControllerUnlockData(participantLogReader):
...
for file in os.listdir(inputDir):
    with(open(os.path.join(inputDir, file), 'r')) as log:
        ...
        pidDataMap[pid] = extractPidControllerUnlockData(reader)
...
with open(os.path.join(outputDir, "Controller_Unlock_Mean_Per_Participant.csv"), 'w') as f:
    writer = csv.writer(f)
    header = ["Condition", "Controller Unlock Time"]
